...
```
The mean total number of steps taken per day is `r format(mean.steps.per.day)`, and the median steps taken per day is `r format(median.steps.per.day)`
...
```
The 5-minute interval, on average across all the days in the dataset, that contains the maximum number of steps is the interval # `r format(interval.with.maximum.number.steps)`
...
```
There are `r format(number.NA)` NA values in the original data set.

The mean total number of steps taken per day is without NA values is `r format(mean.steps.per.day.without.NA)` (original value was `r format(mean.steps.per.day)`), and the median steps taken per day is `r format(median.steps.per.day.without.NA)` (original value was `r format(median.steps.per.day)`)
